...
#### Step 4.2: Aggregate Weather Data by Day and Origin Airport

Initially, a warning occurred due to multiple rows in both datasets matching each other on the same day and airport. To avoid the *many-to-many relationship* issue during the data join, we combine the weather data by year, month, day, and origin. We calculate the daily mean temperature using the `mean(temp, na.rm = TRUE)` function. This ensures that, when joined with `flight` data, there’s a *single temperature value for each origin airport on each day*.

```{r weather-origin}
# Combine the weather data to get the mean temperature per day and origin airport
...
#### Step 4.3: Join Flights with Weather Data
In this step, we filter the flights data to select only the flights that occurred on the day with the highest total departure delay. Then, we perform a left_join() with the aggregated weather data (weather_daily) based on matching year, month, day, and origin.

```{r mean-temp-dep-delay}
# Join the flight data with the combined weather data
mean_temp <- flights %>%
  filter(year == max_dep_delay$year, month == max_dep_delay$month, day == max_dep_delay$day) %>%
  left_join(weather_daily, by = c("year", "month", "day", "origin")) %>%
  distinct(origin, .keep_all = TRUE) %>%  # Remove duplicates if needed
