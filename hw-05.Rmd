...
#### b. How do the total purchases of Lego sets compare between male and female purchasers?

```{r test}
# Calculate total purchases by year
trend_of_purchases <- lego_sales %>%
  group_by(year) %>%  # Group by year
  summarise(total_purchases = sum(quantity, na.rm = TRUE)) %>%  # Sum quantities for total purchases
  arrange(year)

# Create a line graph to visualize the trend of purchases over the years
ggplot(trend_of_purchases, aes(x = year, y = total_purchases)) +
  geom_line(color = "blue", size = 1.2) +  # Line for the trend
  geom_point(color = "red", size = 3) +    # Points for each year
  labs(title = "Trend of Lego Purchases Over the Years",
       x = "Year",
       y = "Total Purchases") +
  theme_minimal() +
